...
function ResetPassword({setIsChangingPassword}:{setIsChangingPassword:React.Dispatch<React.SetStateAction<boolean>>}){
  ...
  const handleNewPasswordSubmit = (data: ResetPasswordData)=>{
  ...
  return(
    <form className={style.change_pass_form} onSubmit={handleSubmit((data)=>handleNewPasswordSubmit(data))}>
...
function ChangeInformation({setIsChangingInformation}:{setIsChangingInformation:React.Dispatch<React.SetStateAction<boolean>>}){
  const {register,handleSubmit,setError,formState:{errors}} = useForm<ChangeInformationData>();
  const handleNewPasswordSubmit = (data: ChangeInformationData)=>{
    console.log(data)
  }
