...
import { updateProfile, changePassword } from "@/app/(main_page)/actions/profile";
...
function ResetPassword({setIsChangingPassword}:{setIsChangingPassword:React.Dispatch<React.SetStateAction<boolean>>}){
  ...
  const onSubmit = async (data: ResetPasswordData) => {
    ...
    if (res.success) {
      ...
      setApiError(res.error ?? 'Password change failed.');
    }
  };

  return(
    <form className={style.change_pass_form} onSubmit={handleSubmit(onSubmit)}>
      ...
      {apiError && <p className={style.error_message}>{apiError}</p>}
      {success && <p className={style.success_message}>Password changed successfully!</p>}
...
interface ChangeInformationData {
...
function ChangeInformation({setIsChangingInformation}:{setIsChangingInformation:React.Dispatch<React.SetStateAction<boolean>>}){
  ...
  const onSubmit = async (data: ChangeInformationData) => {
    ...
    const res = await updateProfile({
      full_name: data.new_full_name || undefined,
      date_of_birth: data.new_birth_date || undefined,
      state: data.location || undefined,
      phone_number: data.new_phone || undefined,
      email: data.new_email || undefined,
    });
    setSubmitting(false);
    if (res.success) {
      setSuccess(true);
  ...
  return (
    <form className={style.user_settings_form} onSubmit={handleSubmit(onSubmit)}>
      <input readOnly onFocus={(e) => e.target.removeAttribute('readonly')} type="text" {...register("new_full_name")} placeholder="Enter new Full name" />
      <input onFocus={(e) => e.target.removeAttribute('readonly')} type="date" {...register("new_birth_date")} />
      <select {...register("location")}>
        <option value="">Select location</option>
        {wilayas.map((wilaya) => <option key={wilaya.code} value={wilaya.name}>{wilaya.name}</option>)}
      </select>
      <input readOnly onFocus={(e) => e.target.removeAttribute('readonly')} type="text" {...register("new_phone")} placeholder="Enter new Phone number" />
      <input readOnly onFocus={(e) => e.target.removeAttribute('readonly')} type="text" {...register("new_email")} placeholder="Enter new Email" />
      {apiError && <p className={style.error_message}>{apiError}</p>}
      {success && <p className={style.success_message}>Profile updated successfully!</p>}
      <div className={style.buttons_container}>
...
export function LogoutPanel({ setOpen, mobile }: { setOpen: React.Dispatch<React.SetStateAction<boolean>>, mobile?: boolean }) {
  ...
  return (
    <div ref={tabRef} className={mobile ? style.user_settings_mobile : style.logout_panel}>
      <div className={mobile ? style.container : style.logout_panel_inner}>
        <button className={style.logout_button} onClick={handleLogout}>Log out</button>
      </div>
      <svg onClick={() => setOpen(false)} viewBox="0 0 24 24" width={24} height={24}>
...
export default function UserSettings({ setSettingsOpen, mobile }: { setSettingsOpen: React.Dispatch<React.SetStateAction<boolean>>, mobile?: boolean }) {
  ...
  return (
    <div ref={tabRef} className={mobile ? style.user_settings_mobile : style.user_settings}>
