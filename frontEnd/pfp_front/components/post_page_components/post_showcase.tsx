        ...
        {screenWidth && (
          <div className={style.post_actions}>
            <div className={style.copy_link_container} onClick={handleCopyLink}>
              ...
              <div className={style.copy_link}>
                {copied ? "Copied!" : "Copy Location Link"}
              </div>
            </div>
            <div
              className={style.save_container}
              onClick={onSaveToggle}
              style={{ cursor: "pointer" }}
            >
              <div className={style.save_logo}>
                {isSaved ? SAVE_LOGO_ACTIVE : SAVE_LOGO_INACTIVE}
              </div>
...
function postImages(pictures: HouseImage[]) {
  return (
    <>
      {pictures.length > 2
        ? picturesMoreThanTwo(pictures)
        : picturesLessThanTwo(pictures)}
    </>
  );
  ...
  return (
    <div className={style.images_navigation}>
      <div className={style.location_details}>
        <div
          className={style.entire_home}
        >{`Entire home: ${formatPostTitle(post_data)}`}</div>
  ...
  return (
    <div className={style.all_pictures_container}>
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button
          className={style.leave_show_pictures}
    ...
    <section className={style.post_showcase}>
      <>
        <PostHeader
          ...
          lat={post_data.location?.Latitude}
          lng={post_data.location?.Longitude}
        />
