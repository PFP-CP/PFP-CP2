...
async function tryRefresh(refresh: string): Promise<{ success: false } | { success: true; access: string }> {
  try {
...
export async function proxy(request: NextRequest) {
  ...
  if (token && isTokenExpired(token)) {
    ...
    if (!refreshed.success) return NextResponse.redirect(new URL('/authentication', request.url))
    needsNewCookie = true
  ...
  if (token && !isAuthRoute && !isVerifyRoute) {
    ...
    if (!verified) {
      try {
        ...
        if (email) {
          const verRes = await fetch(`${API}/api/Account/isUserVerfied?mail=${encodeURIComponent(email)}`, {
  ...
  if (token && !isAuthRoute && !isVerifyRoute && request.cookies.get('email_verified')?.value !== '1') {
