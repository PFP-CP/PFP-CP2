...
class Post(models.Model):
    ...
    def current_tenant(self):
        ...
        res = (
            self.reservations.filter(arrival_date__lte=today, departure_date__gte=today)
            .select_related("renter", "renter__contact")
            .first()
        )
    ...
    def publish(self):
        ...
        if self.status not in PostStatus.PENDING:
            raise ValidationError(f"Cannot publish a post with status '{self.status}'.")
...
class Comment(models.Model):
    ...
    def _update_post_rating(self):
        ...
        avg = Comment.objects.filter(post=self.post, type="post").aggregate(
            avg=Avg("rating")
        )["avg"]
