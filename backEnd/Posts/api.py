...
def list_posts(
    ...
    """
    qs = (
        Post.objects.select_related("house")
        .prefetch_related(
            "house__pictures",
...
@router.get(
    "/{post_id}", response={200: PostOut, 404: ErrorSchema}, tags=["Posts"], auth=None
)
...
@router.post(
    "/", response={201: PostOut, 422: ErrorSchema, 403: ErrorSchema}, tags=["Posts"]
)
@transaction.atomic
...
@router.get(
    "/{post_id}/comments", response=List[CommentOut], tags=["Comments"], auth=None
)
